...
//   isModalOpen,
// }) => {
//   const DEFAULT_CATEGORY: StackCategory = "web";
...
//   const [editDraft, setEditDraft] = useState<StackItemDraft | null>(null);
//   const [newItem, setNewItem] = useState<StackItemDraft>({
//     name: "",
...
const EditStackModal: React.FC<EditStackModalProps> = ({
  ...
  // ---------------- EDIT EXISTING ----------------
  const handleSelectedNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setEditDraft((prev: StackItemDraft) => ({
      ...(prev ?? selectedItem!),
      name: e.target.value,
    }));
  };
  ...
  const submitChangedItem = async () => {
    ...
    const data = editDraft ?? selectedItem;
  ...
  // ---------------- CREATE NEW ----------------
  const handleNewCardName = (e: React.ChangeEvent<HTMLInputElement>) => {
    setNewItem((prev: StackItemDraft) => ({ ...prev, name: e.target.value }));
  };
  ...
  const handleNewCardCategory = (e: React.ChangeEvent<HTMLSelectElement>) => {
    setNewItem((prev: StackItemDraft) => ({
      ...prev,
      category: e.target.value as StackCategory,
    }));
  };
  ...
  const addNewItem = async () => {
    if (!newItem.name || !newItem.category || !thumbnailNewCard) return;

    const formData = new FormData();
    formData.append("name", newItem.name);
    formData.append("category", newItem.category);
    formData.append("img", thumbnailNewCard);
      ...
      <div className="modal-body">
          ...
          {selectedItem ? (
            <div className="edit__or__delete__card">
              <div className="form-group">
                <label>Name</label>
                <input
                  value={editDraft?.name ?? selectedItem.name}
                  onChange={handleSelectedNameChange}
                />
              </div>

              <div className="form-group">
                <label className="thumbnail__label" htmlFor="editThumbnail">
                  Image <EditImageBtn />
                </label>
        ...
        <div className="add-section">
          ...
          <div className="edit__or__delete__card">
            ...
            <div className="form-group">
              <label className="thumbnail__label" htmlFor="newThumbnail">
                Image <EditImageBtn />
              </label>
              ...
              <div className="thumbnail__preview">
                {thumbnailUrlNewCard && (
                  <img
                    src={thumbnailUrlNewCard}
                    alt={thumbnailNewCard?.name || "Preview"}
                    width={thumbnailDimensionsNewCard.width}
                    height={thumbnailDimensionsNewCard.height}
                  />
                )}
            ...
            <div className="form-group">
              <label>Category</label>
              <select value={newItem.category} onChange={handleNewCardCategory}>
                <option value="web">Web</option>
                <option value="mobile-ios">iOS</option>
                <option value="mobile-android">Android</option>
                <option value="ux">UX / UI</option>
            ...
            <div className="btn__group__single">
              <button
                ...
                className="btn-add"
                disabled={
                  !newItem.name || !newItem.category || !thumbnailNewCard
                }
              >
                Add Card
              </button>
